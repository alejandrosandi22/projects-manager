...
export function CustomField({
  getCredentials, handleRemoveField, removedFields, id,
}) {
  return (
    <>
      {
...
export default function CreateProject({ modalsEvents }) {
  ...
  const [createdProject] = useMutation(CREATE_PROJECT, {
    ...
    onError: () => {
      dispatch({
        type: '@alert/show',
        payload: {
          status: true, type: 'error', message: 'An error has occurred', seconds: 5,
        },
      });
    },
    onCompleted: () => {
      dispatch({
        type: '@alert/show',
        payload: {
          status: true, type: 'success', message: 'Project created successfully', seconds: 5,
        },
      });
      closeModal();
    },
  ...
  const [editProject] = useMutation(EDIT_PROJECT, {
    ...
    onCompleted: () => {
      dispatch({
        type: '@alert/show',
        payload: {
          status: true, type: 'success', message: 'Project updated successfully', seconds: 5,
        },
      });
      closeModal();
    },
  ...
  const handleSubmit = (e) => {
    ...
    if (createProject.functionality === 'create') return createdProject({ variables: { ...organizedData } });
    return editProject({ variables: { ...organizedData, id: data.id } });
  };
