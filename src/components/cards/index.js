  ...
  const [completeProject] = useMutation(COMPLETE_PROJECT, {
    ...
    onCompleted: () => {
      dispatch({
        type: '@alert/show',
        payload: {
          status: true, type: 'success', message: 'Successfully', seconds: 5,
        },
      });
    },
  });
  ...
  return (
    <div className={styles.card}>
      <div className={styles.header}>
        ...
        <ul>
          <li onClick={() => openEditProjectsModal(props)} className={`fas fa-edit ${styles.edit}`} />
          <li onClick={() => openDeleteProjectModal(props.id, props.name)} className={`fas fa-trash ${styles.trash}`} />
        </ul>
      </div>
